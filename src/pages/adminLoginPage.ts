...
<title>Ðang nh?p Admin – QH Clothes</title>
  ...
  <div class="fade-up w-full max-w-md">
    ...
    <div class="glass-card rounded-3xl p-8" id="loginCard">
      <h2 class="text-white text-xl font-bold mb-6 text-center">
        <i class="fas fa-lock text-pink-400 mr-2"></i>Ðang nh?p qu?n tr?
      </h2>
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-gray-300 text-sm font-medium mb-2"><i class="fas fa-user text-pink-400 mr-1"></i>Tên dang nh?p</label>
          <input type="text" id="loginUsername" placeholder="Nh?p tên dang nh?p" class="input-dark w-full px-4 py-3 rounded-xl text-sm" autofocus>
        </div>
        <div>
        ...
        <button onclick="doLogin()" id="loginBtn" class="btn-login w-full text-white py-3.5 rounded-xl font-bold text-sm mt-2">
          <i class="fas fa-sign-in-alt mr-2"></i>Ðang nh?p
        </button>
      </div>
    </div>
...
<script>
  ...
  async function doLogin() {
    ...
    if (!username || !password) {
      errText.textContent = 'Vui lòng nh?p d?y d? thông tin'
    ...
    try {
      ...
      errText.textContent = 'Sai tên dang nh?p ho?c m?t kh?u'
      errEl.classList.remove('hidden')
      card.classList.remove('shake'); void card.offsetWidth; card.classList.add('shake')
    } finally {
      btn.disabled = false
      btn.innerHTML = '<i class="fas fa-sign-in-alt mr-2"></i>Ðang nh?p'
    }
  }
</script>
</body>
