...
function closeUserMenu() {
  ...
  setTimeout(() => { overlay.classList.add('hidden'); panel.classList.remove('closing'); syncBodyScrollLock() }, 300)
}
...
function syncBodyScrollLock() {
  ...
  const shouldLock = visibleOverlayIds.some(function (id) {
    const el = document.getElementById(id)
...
function getOrderHistoryLifecycle(order) {
  ...
  if (orderStatus === 'done') {
    return { key: 'done', label: 'Đã nhận', icon: 'fa-circle-check', toneClass: 'order-status-chip--done', clickable: false, hasJourney: true }
  }
  if (orderStatus === 'shipping') {
    return { key: 'shipping', label: 'Đang giao', icon: 'fa-truck-fast', toneClass: 'order-status-chip--shipping', clickable: hasTracking, hasJourney: true }
  }
  if (shippingArranged || hasTracking) {
    return { key: 'waiting_pickup', label: 'Đang chờ lấy hàng', icon: 'fa-truck-ramp-box', toneClass: 'order-status-chip--waiting-pickup', clickable: hasTracking, hasJourney: true }
  }
...
function buildShippingJourneySteps(order) {
  const lifecycle = getOrderHistoryLifecycle(order)
  const steps = [
    { key: 'waiting_shop', label: 'Shop duyệt đơn' },
    { key: 'waiting_pickup', label: 'Đang chờ lấy hàng' },
  ...
  return { lifecycle, steps, activeIndex: activeIndexMap[lifecycle.key] ?? 0 }
}

function renderShippingJourneyModal(order) {
  const content = document.getElementById('shippingJourneyContent')
  if (!content) return
  const info = buildShippingJourneySteps(order)
  const trackingCode = String(order?.shipping_tracking_code || '').trim()
  const journeyNote = info.lifecycle.key === 'waiting_shop'
    ? 'Đơn đang chờ shop duyệt.'
    : info.lifecycle.key === 'waiting_payment'
      ? 'Đơn chưa hoàn tất thanh toán.'
      : info.lifecycle.key === 'cancelled'
        ? 'Đơn đã bị hủy.'
        : info.lifecycle.key === 'done'
          ? 'Đơn đã được giao thành công.'
          : 'Đơn đã được đẩy sang đơn vị vận chuyển.'

  const stepsHtml = info.steps.map(function (step, idx) {
    const isComplete = info.activeIndex >= idx
    const isActive = info.activeIndex === idx
    const cls = isActive ? 'is-active' : isComplete ? 'is-complete' : ''
    return '<div class="shipping-journey-step ' + cls + ' pb-4">'
      ...
      + '<p class="font-semibold text-gray-800">' + escapeHtml(step.label) + '</p>'
  ...
  content.innerHTML = ''
    ...
    + '</div>'
    + '<span class="order-status-chip ' + info.lifecycle.toneClass + '"><i class="fas ' + info.lifecycle.icon + '"></i>' + escapeHtml(info.lifecycle.label) + '</span>'
    + '</div>'
