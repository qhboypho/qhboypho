...
assert.match(modalsSource, /onclick=\\\"if\(event\.target === this\) closeOrderDetailModal\(\)\\\"/, 'order detail backdrop should use the shared close helper')
assert.match(modalsSource, /onclick=\\\"closeOrderDetailModal\(\)\\\" class=\\\"w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center\\\"/, 'order detail close button should use the shared close helper')
